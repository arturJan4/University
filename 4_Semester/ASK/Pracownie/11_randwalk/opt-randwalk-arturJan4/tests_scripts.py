...
# times (t) and n, s is log_2
# S is seed, s -steps
def run_randwalk(p='l1', n='7', seed='0xea3495cc76b34acc', steps='12', t='10',  v='0'):
    parameters = ['-p', p, '-S', seed, '-n', n, '-t', t, '-s', steps, '-v', v]
...
def time_to_n():
    output = "n, v0, v1\n"
    n_possible = [str(i) for i in range(1, 16, 1)]
    ...
    for n in n_possible:
        ...
        for variant in range(0, num_variants):
            ...
            for i in range(1, howMany+1):
                time, mr = run_randwalk(n=n, steps='9', v=str(variant))
...
def ipc_to_n():
    ...
    for n in n_possible:
        ...
        for variant in range(0, num_variants):
            IPC = 0.0
            for i in range(1, howMany+1):
                instr, ipc = run_randwalk(p='ipc', n=n, v=str(variant))
                IPC += float(ipc[0])
            output += ', ' + str(IPC/howMany)

        output += '\n'

    out_file = open('data_ipc_to_n4312.dat', 'w')
    out_file.write(output)
    out_file.close()
...
def get_average(p='l1', n='15', s='0x5bab3de5da7882fb', t='10', steps='12', how_many=10):
    ...
    for variant in range(0, num_variants):
        ...
        for i in range(1, how_many + 1):
            first, second = run_randwalk(p=p, n=n, seed=s, steps=steps, t=t, v=str(variant))
...
def averages_table():
    ...
    out_file = open('data_averages.dat', 'w')
    out_file.write(output)
    out_file.close()
    print("saved to data_averages.dat")
