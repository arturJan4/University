...
(define (eval-env e env global-env)
  (match e
    [(const n) n]
    [(binop op l r) ((op->proc op) (eval-env l env global-env)
                                   (eval-env r env global-env))]
    [(let-expr x e1 e2)
     (eval-env e2 (env-add x (eval-env e1 env global-env) env) global-env)]
    [(letrec-expr x e1 e2)
     (let* ([new-env (env-add x (blackhole) env)]
            [v (eval-env e1 new-env global-env)])
       (begin
          (env-update! x v new-env)
          (eval-env e2 new-env global-env)))]
          
    [(var-expr x) (let ([r1 (env-lookup x env)])
                    (if (null? r1)
                        (let ([r2 (env-lookup x global-env)])
                          (if (null? r2)
                              (error "Unidentfied variable" x) ;;<---------------------
                              r2))
                        r1))]
                        
    [(if-expr eb et ef) (if (eval-env eb env global-env)
                            (eval-env et env global-env)
                            (eval-env ef env global-env))]
    [(cons-expr e1 e2) (cons (eval-env e1 env global-env)
                             (eval-env e2 env global-env))]
    [(car-expr e) (car (eval-env e env global-env))]
    [(cdr-expr e) (cdr (eval-env e env global-env))]
    [(null-expr) null]
    [(null?-expr e) (null? (eval-env e env global-env))]
    [(lam x e) (clo x e env)]
    [(app f e)
     (let ([vf (eval-env f env global-env)]
           [ve (eval-env e env global-env)])
       (match vf [(clo x body fun-env)
                  (eval-env body (env-add x ve fun-env) global-env)]))]))
...
#|
Pomysł:
Potraktujmy zmienne WHILE-owe jako zmienne globalne, a zmienne które wylicza funkcja eval jako lokalne i np:
Dla assign x := e, liczymy e w pustym środowisku z dostępem do zmiennych globalnych (referencji)
Gdy natrafimy w evalu na zmienną (var-expr) to szukamy jej najpierw w środowisku lokalnym, a dopiero potem globalnym
|#

;;testy (wystarczy zmienic numer testu w runTEST)
;;nr 1 (10)
(define TEST1
  '{(x := 5)
    (f := (lambda (y) (+ x y)))
