...
(struct apply      (fun l)    #:transparent) ;<----------
...
(define (expr? e)
  (match e
    ...
    [(apply fun l) (and (expr? fun) (expr? l))]  ;<----------
    [_ false]))
...
(define (parse q)
  (cond
    ...
    [(and (list? q) (eq? (length q) 3) (eq? (first q) 'let))
     ...
     [(and (list? q) (eq? (first q) `apply) (= 3 (length q)))
     (apply (parse (second q))
                   (parse (third q)))]
...
(define (eval-env e env)
  (match e
    ...
    [(apply proc l)
     (let ([ev-l (eval-env l env)]
           [ev-proc (eval-env proc env)])
       (helper-apply ev-proc ev-l))]))

(define (helper-apply proc xs)
  (if (null? xs)
      proc
      (match proc [(clo x body fun-env)
       (helper-apply (eval-env body (env-add x (car xs) fun-env)) (cdr xs))]
        [_ (error "Error")])))
  
(define (eval e) (eval-env e env-empty))

(eval (parse
  '(apply + (cons 2 (cons 1 null)))))
;;testy
(eval (parse
...
(eval (parse `(let (x 0) (let (y (apply (lambda (x y) (+ x y)) (list 5 6))) (* x y)))))