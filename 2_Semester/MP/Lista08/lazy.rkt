  ...
  (match e
    ...
    [(var-expr x)
     (match (env-lookup x env)
       [(postpone e env) (eval-env e env)]
       [v v])]
    [(if-expr eb et ef) (if (eval-env eb env)
                            (eval-env et env)
                            (eval-env ef env))]
    [(cons-expr e1 e2) (cons (postpone e1 env)
                             (postpone e2 env))]
...
(eval (parse '(+ 1 1)))
(eval (parse '((lambda (x) (+ 3 x)) (+ 2 1))))
(eval (parse '((lambda (x y) (+ y y)) (/ 1 0) 2)))
(eval (parse '(if (let (x 2)
                    (if (null? x) true true))
                       ((lambda (x) (+ 7 8)) (+ 9 9)) (/ 0 0))))
(eval (parse '(if true
                  42
                  (if (null? (/ 1 0)) 0 1))))

(eval (parse '(let (test (cons ((lambda (x) 8) (+ 3 3)) (cons ((lambda (x) (/ 5 x)) 0)  null))) (if (null? (car test)) (/ 5 0) (cdr (cdr test))))))